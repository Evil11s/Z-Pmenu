...
local defaultMenu = { { name = "Rien" } }
local _intX, _intY = .28, .175
local _intW, _intH = .260, .033
local spriteW, spriteH = .260, .0600
local PMenu = {}
local parentSliderSize = .26 
...
function PMenu:OpenMenu(stringName, boolBack)
	if stringName and not self.Menu[stringName] then 
		return end
...
function PMenu:CreateMenu(tableMenu, tempData)
	...
	if not self.IsVisible and tableMenu then
		...
		_intY = self.Base.CustomHeader and .205 or .30
...
function PMenu:CreateMenuPeds(tableMenu, tempData, peds)
	if (self.Base and self.Base.Blocked and self.IsVisible and IsMenuOpened()) or not tableMenu then return end
	if not self.IsVisible and tableMenu then
		self:resetMenu()
		tableMenu.Base = tableMenu.Base or {}
		...
		local ped = GetClosestPed2(GetEntityCoords(GetPlayerPed(-1)),5.0)
		if ped then 
...
function PMenu:ProcessControl()
	...
	if IsControlJustPressed(1, 177) and UpdateOnscreenKeyboard() ~= 0 then
		self:Back()
		Citizen.Wait(100)
	end
...
function PMenu:drawMenuButton(button, intX, intY, boolSelected, intW, intH, intID) --6,48,200
	local tableColor, add, currentMenuData = boolSelected and (button.colorSelected or { 255, 255, 255, 255 }) or (button.colorFree or { 0, 0, 0, 180 }), .0, self.Menu[self.Data.currentMenu]
	...
	if not button.separator then
		DrawText2(0, button.name or "", .275, intX - intW / 2 + .005, intY - intH / 2 + .0025, tableColor)
	else
		DrawText2(4, (button.sepColor or "") .. (button.separator or ""), .375, intX - string.len(button.separator)/750, intY - intH / 2 + .0005, tableColor, true, true)	
	end
	...
	local slideExist = slide and slide.slidemax and (not slide.slidefilter or not tableValueFind(slide.slidefilter, intID))
	...
	elseif slideExist or button.ask or button.slidename then
		...
		if (max and type(max) == "number" and max > 0 or type(max) == "table" and #max > 0) or not slideExist then
			...
			if boolSelected and slideExist then
				...
				button.offset = MeasureStringWidth(slideText, 0, .215)
			...
			DrawText2(4, slideText, .375, intX + intW / 2 + textX,  intY - intH / 2 + .00075, tableColor, false, 2)
...
function PMenu:DrawButtons(tableButtons)
	local padding, pd = 0.0165, 0.0455
...
function PMenu:DrawBanner(tableButtons)
	RequestStreamedTextureDict("properties_thumbnails", 1)
	while not HasStreamedTextureDictLoaded("properties_thumbnails") do
		Wait(50)
		print("wait texture")
	end
	...
	if button then 
		if button.sprite then
			local spriteH = .21
			local intX, intW, intY, intH = 0.1174, 0.24, 0.435, 0.028
			local proW, proH = _intW, 0.055
			self.Height =  self.Height - 0.01 + 0.075
			drawSprite("properties_thumbnails", button.sprite, self.Width - proW / 2, self.Height + proH / 2, spriteW, spriteH, 0.0, 255, 255, 255, 255)
		end
	end
end
...
function PMenu:DrawHeader(intCount)
	...
	if boolHeader then
		...
		drawSprite(parentHeader, childHeader, self.Width - spriteW / 2, self.Height - intH / 2, spriteW, intH, .0, 140, 194, 255, 180)
		self.Height = self.Height - 0.03
		if not self.Base.CustomHeader then
			DrawText2(4, self.Base.Title, .7, self.Width  - spriteW / 2, self.Height - intH / 2 + .009, color_white, false, 0)
		end
	end
	...
	self.Height = self.Height + 0.005
	DrawText2(0, stringCounter, .275, self.Width - rectW / 2 + .128, self.Height - (_intH + 0.0015), color_white, true, 2)
	if currentMenu and currentMenu.charCreator then
		local spriteW, spriteH = .240, .21
...
function PMenu:DrawHelpers(tableButtons)
	...
	if hasHelp then
		...
		local nwintH = intH + (Linecount == 1 and 0 or ( (Linecount + 1) * 0.0045))
		...
		DrawText2(0, descText, scale, self.Width - _intW + .005, self.Height - 0.0120, color_white)
	end
end
...
function PMenu:DrawExtra(tableButtons)
	...
	if button and button.advSlider ~= nil then
		...
		if button.advName then
			DrawText2(0, button.advName .. " (" .. button.advSlider[3] .. ")", 0.275, self.Width - _intW / 2, self.Height, color_white, false, 0)
		else
			DrawText2(0, "Variations disponibles (" .. button.advSlider[3] .. ")", 0.275, self.Width - _intW / 2, self.Height, color_white, false, 0)
		end
...
function CreateMenuPeds(arrayMenu, tempData)
	return PMenu:CreateMenuPeds(arrayMenu, tempData)
end
...
function CreateRoue(currentMenu, cam, ped)
	...
	TriggerEvent("clp:toggleroue", menuData, cam, ped)
end
